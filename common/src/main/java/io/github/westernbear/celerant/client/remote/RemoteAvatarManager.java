...
public final class RemoteAvatarManager {
	...
	public static void onMeta(CelerantNetworking.AvatarMetaPayload meta) {
		Incoming inc = incoming.computeIfAbsent(meta.avatarId(), id -> new Incoming(meta));
		inc.meta = meta;
	}

	public static void onChunk(CelerantNetworking.AvatarChunkPayload chunk) {
		Incoming inc = incoming.computeIfAbsent(chunk.avatarId(),
			id -> new Incoming(new CelerantNetworking.AvatarMetaPayload(chunk.ownerId(), chunk.avatarId(),
				new byte[32], 0, 0)));
	...
	public static void onKey(CelerantNetworking.AvatarKeyPayload key) {
		Incoming inc = incoming.get(key.avatarId());
		if (inc == null) {
			inc = new Incoming(new CelerantNetworking.AvatarMetaPayload(key.ownerId(), key.avatarId(),
				new byte[32], 0, 0));
	...
	public static boolean uploadLocal(Path vrmPath) {
		...
		try {
			...
			CelerantNetworking.sendMeta(new CelerantNetworking.AvatarMetaPayload(owner, avatarId, hash,
				envelope.length, 0));
			...
			for (int offset = 0, index = 0; offset < envelope.length; index++) {
				...
				CelerantNetworking.sendChunk(new CelerantNetworking.AvatarChunkPayload(owner, avatarId, streamId,
					index, slice, last));
			}
			onKey(new CelerantNetworking.AvatarKeyPayload(owner, avatarId, key));
			return true;
		} catch (Exception e) {
	...
	private static final class Incoming {
		CelerantNetworking.AvatarMetaPayload meta;
		byte[] sessionKey;
		...
		Incoming(CelerantNetworking.AvatarMetaPayload meta) {
			this.meta = meta;
		}
